...
def adjust_payload_for_new_bucket(dump_truck_payload, new_payload):
    ...
    while current_payload <= max_payload:
        ...
        if abs(swings_to_fill_truck_new - math.ceil(swings_to_fill_truck_new)) <= 0.14:
            st.write(f"*Dump truck fill factor of {(100*current_payload/dump_truck_payload)}% applied for XMOR® bucket pass matching.")
...
def adjust_payload_for_old_bucket(dump_truck_payload, old_payload):
    ...
    while current_payload <= max_payload:
        ...
        if abs(swings_to_fill_truck_old - math.ceil(swings_to_fill_truck_old)) <= 0.14:
            st.write(f"*Dump truck fill factor of {(100*current_payload/dump_truck_payload)}% applied for Old bucket pass matching.")
